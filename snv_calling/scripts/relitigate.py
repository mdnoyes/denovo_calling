...
class Variant:
	...
	def __init__(
		...
		self.old_validation = variant_data[file_header.index('validation')]
		self.tandem_repeat = eval(variant_data[file_header.index('in_tr')])
		self.concordant_ab = eval(variant_data[file_header.index('concordant_ab')])
	...
	def find_missing_support(self, platform: str) -> str:
		...
		for platform in ['hifi', 'ont', 'illumina']:
			if self.allele_counts[platform]['dad'][1] > threshold or self.allele_counts[platform]['mom'][1] > threshold:
				return 'inherited'

		if self.noise_in_parent('mom') or self.noise_in_parent('dad'):
			return 'too_noisy_to_call'

		if self.allele_counts[platform]['child'][1] > 0:
			return 'true_de_novo'

		if platform != 'hifi' and self.concordant_ab and self.allele_balance > 0.05:
				return 'true_de_novo'
	...
	def relitigate(self) -> str:
		...
		if self.tandem_repeat:
			...
			if self.allele_balance < 0.05:
				return 'tandem_repeat_error'

		# variants with discordant AB must have AB of at least 5%
		if not self.concordant_ab and self.allele_balance < 0.05:
		...
		# if a variant has at least 3 long reads coming from one parent, it should be considered inherited (based on IGV validation)
		for parent in ['mom', 'dad']:
			if self.allele_counts['hifi'][parent][1] > 0 and self.allele_counts['ont'][parent][1] > 0 and (self.allele_counts['hifi'][parent][1] + self.allele_counts['ont'][parent][1]) >= 3:
				print(self.id, self.allele_balance)
				print((self.allele_counts['hifi'][parent][1] + self.allele_counts['ont'][parent][1]))
				return 'inherited'
...
def main(variant_file, read_file, father, mother, child, output_file):
	samples = {father: 'dad' , mother: 'mom', child: 'child'}
