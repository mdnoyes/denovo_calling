...
def get_sample_list(bams, family, child):
	for idx, row in bams.iterrows():
		sm = row["sample"]; fm = row["family"]
		if child == sm and family == fm:
			fa_id = row['father']
			family_list = [fa_id, f'{family}_{child}']
...
def main(variants, bams, family, platform, denovo_sample):
	...
	samples = get_sample_list(bam_df, family, denovo_sample)
	print(samples)

	reads_out = open(f'read_data/chrY_no_PAR/{family}_{denovo_sample}_{platform}.read_data.tsv', 'w')
	samples_out = open(f'read_data/chrY_no_PAR/{family}_{denovo_sample}_{platform}.read_summary.tsv', 'w')
	...
	for sample_id in samples:
		...
		for idx, row in tqdm(variant_df.iterrows(), total=len(variant_df)):
			if row[0] == 'chr':
				continue
			get_snv_data(sample_bam, row[2], row[0], row[1], row[3], row[4], reads_out, samples_out)
...
if __name__ == "__main__":
	...
	main(snakemake.input.snvs, snakemake.input.bam_manifest, snakemake.params.family, snakemake.params.platform, snakemake.params.child)
	# main(args.input, args.bams, args.family, args.platform, args.child)
