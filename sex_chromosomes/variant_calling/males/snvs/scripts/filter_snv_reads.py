...
def determine_false_positive(allele_stats: Tuple[int, int, float]):
	"""
...
def check_biallelic(blood_stats: Tuple[int, int, float], cell_stats: Tuple[int, int, float]):
	"""
	Determine if variant is biallelic.
	:param blood_stats: Tuple with child blood alt allele count, read depth, and allele balance.
	:param cell_stats: Tuple with child blood alt allele count, read depth, and allele balance.
	:return: True if site is biallelic, false if not.
	"""

	if blood_stats[0] > 0 and blood_stats[2] < 0.95:
		return True

	if platform == 'hifi':
		if cell_stats[0] > 0 and cell_stats[2] < 0.5:
			return True

	if platform == 'ont':
		if cell_stats[0] > 0 and cell_stats[2] < 0.9:
			return True

	return False

def validate_variant(blood_summary: Tuple, cell_summary: Tuple, biallelic_status: bool, parent_summary: Tuple):
	"""
	Determine if variant is true, inherited, false, or missing data.
	...
	:param biallelic_status: True/False if alt is biallelic in child data.
	:param parent_summary: Tuple of True/False if parent has data, True/False if alt in high qual reads, True/False if alt in low qual reads.
...
def main(variant_file, read_file, hifi_sources, parent, child, output_file):

	hifi_source_dict = import_sources(hifi_sources)
	variant_reads = import_reads(read_file)

	if platform == 'hifi':
		out_columns = ['chr', 'pos', 'id', 'ref', 'alt', 'callers',
					'child_cell_ref_count', 'child_cell_alt_count', 'child_cell_allele_balance',
					'child_blood_ref_count', 'child_blood_alt_count', 'child_blood_allele_balance',
					'filtered_status']
	else:
		out_columns = ['chr', 'pos', 'id', 'ref', 'alt', 'callers',
					'child_ref_count', 'child_alt_count', 'child_allele_balance', 'child_depth',
					'filtered_status']
	...
	with open(variant_file, 'r') as variants, open(output_file, 'w') as outfile:
		...
		for line in variants:
			...
			bialleleic = check_biallelic(blood_stats, cell_stats)

			validation = validate_variant(blood_dnm, cell_dnm, bialleleic, parent_inh_summary)
...
if __name__ == "__main__":
	...
	# ap.add_argument("-f", "--parent", required=True, help="parent ID", type=str)
	...
	# main(args.variants, args.reads, args.source, args.parent, args.child, args.output)
