...
def get_status(dad_count, child_count, child_dp, pf):
	if pf == 'hifi':
	...
	if np.nan in dad_count:
		return "missing_parent_data"
	if child_dp == 0:
		return "missing_child_data"
	if dad_count[1] > inherited_threshold:
		return "inherited"
	if child_count[1] >= fp_threshold and child_count[0] >= 2:
		return "biallelic"
	if dad_count[0] < inherited_threshold:
		return "missing_parent_data"

	if dad_count[1] <= inherited_threshold and child_count[1] >= fp_threshold:
		return "true_de_novo"
...
if __name__ == "__main__":
	...
	sample_ids = {'dad': snakemake.params.father, 'child': snakemake.params.child}
	...
	with open(snakemake.input.snvs, 'r') as variants, open(snakemake.output.validated, 'w') as outfile:
		out_columns = ['chr', 'pos', 'id', 'ref', 'alt', 'callers', 'dad_ref_count', 'dad_alt_count', 'child_ref_count', 'child_alt_count', 'child_allele_balance', 'child_depth', 'filtered_status']
			...
			for sample_name, sample_id in sample_ids.items() :
				...
				if sample_name == 'dad':
					if len(filtered_reads) > 0:
						...
						dad_count = (num_ref_alleles, num_alt_alleles)
					else:
						dad_count = (np.nan, np.nan)
				else:
					child_count, child_ab, child_dp = get_allele_stats(filtered_reads, ref, alt, read_header)

			# validation = get_status(dad_count, child_count, child_dp, args.platform)
			validation = get_status(dad_count, child_count, child_dp, snakemake.params.platform)
			variant_info += [str(x) for x in [dad_count[0], dad_count[1], child_count[0], child_count[1], child_ab, child_dp, validation]]
