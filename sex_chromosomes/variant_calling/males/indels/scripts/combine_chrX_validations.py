...
def import_manifest(manifest, child):
	...
	return {k: sample_dict[k] for k in samples}
...
def check_hifi_de_novo_allele(mom_alleles, child_alleles):
...
def check_de_novo_allele(mom_alleles, child_alleles, platform):
...
def get_platform_validations(variant_alleles):
	hifi = check_hifi_de_novo_allele(variant_alleles.mom.hifi,
		# variant_alleles.mom.hifi,
		variant_alleles.child.hifi)
	ilm = check_de_novo_allele(variant_alleles.mom.ilm,
		# variant_alleles.mom.ilm,
		variant_alleles.child.ilm, 'illumina')
	ont = check_de_novo_allele(variant_alleles.mom.ont,
...
def validate_indels(read_data_files, family_dict, sample_dict, out_columns):
	...
	for variants in zip(*read_data_files):
		...
		if var_data.mom and var_data.child:
