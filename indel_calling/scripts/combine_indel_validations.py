...
def check_hifi_de_novo_allele(dad_alleles, mom_alleles, child_alleles):
	...
	mom = check_parent(mom_alleles, 'hifi')
...
def check_de_novo_allele(dad_alleles, mom_alleles, child_alleles, platform):
	dad = check_parent(dad_alleles, platform)
	mom = check_parent(mom_alleles, platform)
...
def validate_indels(read_data_files, family_dict, sample_dict, out_columns):
	...
	for variants in zip(*read_data_files):
		...
		if var_data.dad and var_data.mom and var_data.child:
			...
			out_data = [variants[0][1][x] for x in out_columns[:8]] + [validation]
...
def main(data_files, manifest, father, mother, sample, output):
	family_ids = {
		father: 'dad',
		mother: 'mom',
		sample: 'child'
	}
	samples = import_manifest(manifest, sample)
	variant_files = {get_platform(path): path for path in data_files}
	...
	out_columns = ['chr', 'pos', 'id', 'ref', 'alt', 'caller', 'in_tr', 'indel_size', 'validation']
...
if __name__ == "__main__":
	...
	# args = ap.parse_args()
	#
	# main(args.input, args.manifest, args.father, args.mother, args.sample, args.output)
	main(snakemake.input.read_summary, snakemake.input.manifest, snakemake.params.father, snakemake.params.mother, snakemake.params.sample, snakemake.output.validated_indels)
